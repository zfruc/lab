...
int HashTab_Init()
{
    ...
    if(stat == 0)
    {
        ...
        hashitem_freelist->remaining = NTABLE_SSD_CACHE;
        hashitem_freelist->insertCnt = hashitem_freelist->deleteCnt = 0;
    }
    else
    {
...
static SSDBufHashBucket* bucket_alloc()
{
    ...
    long freeId = hashitem_freelist->next_item_id;
    SSDBufHashBucket* freebucket = &hashitem_freelist[freeId];
